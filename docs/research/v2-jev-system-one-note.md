...
- **一次確認**: **A** = 親検収済(逐語引用または再計算つき) — 親の一次読み(2026-09-18・第238): ブログ・docs 5 頁・評価ページ・adapter README 実読+二次 4 本。第242 で R-40 の訂正を反映
...
- **何をするか**: 「非構造の状態(state)を入れ、**型付きの確率的な判断**を返す関数呼び出し」。**文字列を生成しない**。出力は 3 つの原始型だけ。
  ...
  - **(第242 訂正・R-40)** 文脈長は **64k tok/リクエスト・state+最長質問 32k**、レート制限は **250,000 tok/s かつ 1,200 req/min**(動的に変わりうる)、SDK 既定 timeout 10 s、入力は文字列/JSON/配列、**1 リクエスト=1 state**(体を束ねられない)、バッチ API なし=`models.md` に公開済み。モデル規模・アーキテクチャ・RLCD の報酬は非公開。**日本語は「精度が落ちる」と明記**。弱点一覧(jaggedness: 大きな state で context rot・数と日付・否定/間接)もベンダー自身が公開。
...
- **外部 API**: 状態文が米国のサーバーへ出る。体は合成なので個人情報は無いが、**封印データや現実データの値を state に含めない**規律が要る(MCA 4.3 は Telemetry=要約統計・分類・learnings を無制限に処理と定める・R-40)。**(第242 訂正)** レート制限は 1,200 req/min=現行腕(5,000 体・約 37,000 呼を 40 分)は 1,250 req/min で**わずかに超過**、120 分なら余裕。費用は現行呼数で約 $2/シミュ日。**MCA §2.3(f) がベンチマーク/性能情報の公表を禁止**=腕の結果を答申・Discord に出すことの可否は法務判断。オンプレ不可・SLA なし・決定論の保証なし(温度/seed なし)。艦隊(借用 GPU)は現状ほぼ無料なので費用の比較軸が違う。
