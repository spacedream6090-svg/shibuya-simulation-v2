...
| R-38 | **非 LLM モデルとの混成**(親提案・第238): 社会シミュで LLM と学習方策を混ぜた先行と帰属の扱い・世界モデル(JEPA/Dreamer)を知覚や起床に使った例・LLM エージェントの小モデル蒸留で失われる多様性の実測・予測誤差(驚き)で注意を配る計算モデル | — | **未起動**(ユーザー判断待ち。R-39 が Type 1/2 と門番の先行を先に埋めた) |
