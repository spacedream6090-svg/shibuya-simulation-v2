...
| R-35 | **LLM のハーネス**(ユーザー案・第227): 4 分類(A 呼び出し内の多段ループ・ツール呼=未カバー / B 出力の型と再試行=実装済み / C 記憶・内省・階層=認知設計で決定済み / D ラン外バッチ=D-71)。問い=先行の社会シミュでハーネスの何が行動を動かしたか・7〜8B の多段/ツール呼の成功率・呼数 ×k vs モデル拡大・記録粒度・L4/W1 の算術。出口=ablation AB8 の腕定義 | — | 提案・ユーザー判断待ち(意味 A/C・目的・昇格呼のみ・起動) |
...
| R-37 | **行き先選択に LLM の思考を入れる(候補提示型)**(ユーザー意向・D-88 ④・第228): 候補 k を見せて選ばせる先行(LLMob・CoPB・AgentMove 等)・8B のメニュー効果との関係・理由の記録と検証・エンジン単独(EPR/Dubé)との ablation 設計 | — | 候補・行き先選択の設計ラウンド前に(確認待ち) |
| R-23 第3批 | **D 等級だけを根拠にする決定台帳 18 行**の一次確認(D-86 (a)・第228) | [v2-r23-primary-check-batch3.md](v2-r23-primary-check-batch3.md)(予定) | 実行中(第228・サブ Opus) |
