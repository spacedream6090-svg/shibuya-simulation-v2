...
| R-37 | **行き先選択に LLM の思考を入れる(候補提示型)**(ユーザー意向・D-88 ④・第228): 候補 k を見せて選ばせる先行(LLMob・CoPB・AgentMove 等)・8B のメニュー効果との関係・理由の記録と検証・エンジン単独(EPR/Dubé)との ablation 設計 | [v2-destination-choice-llm-research.md](v2-destination-choice-llm-research.md)(予定) | **実行中(第229・ユーザー「回して」・サブ Opus)** |
