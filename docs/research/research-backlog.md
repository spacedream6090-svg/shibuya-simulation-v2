...
| R-35 | **LLM のハーネス**(ユーザー案・第227): 4 分類(A 呼び出し内の多段ループ・ツール呼=未カバー / B 出力の型と再試行=実装済み / C 記憶・内省・階層=認知設計で決定済み / D ラン外バッチ=D-71)。問い=先行の社会シミュでハーネスの何が行動を動かしたか・7〜8B の多段/ツール呼の成功率・呼数 ×k vs モデル拡大・記録粒度・L4/W1 の算術。出口=ablation AB8 の腕定義。**第229 追加の問い**: 呼数の目標(いま 6.9〜7.4 呼/体/日)を現実の活動切替回数・トリップ数・思考遷移数(一次が要る)にアンカーする=D-70 の「思考の頻度を増やしたい」の物差し | — | 提案・ユーザー判断待ち(意味 A/C・目的・昇格呼のみ・起動) |
| R-36 | **C10 初期関係のウォームアップ+関係強度の記憶からの導出**(ユーザー問い・第228): tie strength の履歴からの導出(Granovetter/Gilbert & Karahalios/Saramäki)・ACT-R 基底活性の転用・気分印(語)の信頼性・LLM 社会シミュの初期関係網の与え方・ABM の網生成と burn-in・日本の職場/近隣紐帯の一次・費用算術(40 万体 × k 会話)・指紋の比較・5,000 体の腕定義 | [v2-c10-initial-relations-research.md](v2-c10-initial-relations-research.md)(親確認 3: Gilbert 2009・Zhao 2012・Li 2026+実物 2) | **完了(第230)**: 問 A=②は基底活性で消える(残り 符号・τ・粒度)・①は 3 値を LLM に書かせず ResultCode から・問 B=先行 0 本・世界内は会話率で不可→ 機械的初期化+5,000 体の強制ペア腕・齟齬 6・空欄 10・lit 8。判断=D-93 の 4 分岐 |
| R-37 | **行き先選択に LLM の思考を入れる(候補提示型)**(ユーザー意向・D-88 ④・第228): 候補 k を見せて選ばせる先行(LLMob・CoPB・AgentMove 等)・8B のメニュー効果との関係・理由の記録と検証・エンジン単独(EPR/Dubé)との ablation 設計 | [v2-destination-choice-llm-research.md](v2-destination-choice-llm-research.md)(親確認 3+実物 1) | **完了(第232)**: H の向きは先行と逆・唯一の直接比較は集計で重力則勝ち/個体で LLM 勝ち・8B は順序で 2.6 倍・k=3〜5 一次あり・渋谷は複数セル跨ぎ必須・合否は P-6/P-8・腕 A0〜A4・空欄 10・lit 7。副産物 nightlife 259 件マスク外(W6 サブへ)。判断=D-88 ④ の設計ラウンド |
| R-38 | **非 LLM モデルとの混成**(親提案・第238): 社会シミュで LLM と学習方策を混ぜた先行と帰属の扱い・世界モデル(JEPA/Dreamer)を知覚や起床に使った例・LLM エージェントの小モデル蒸留で失われる多様性の実測・予測誤差(驚き)で注意を配る計算モデル | — | **未起動**(ユーザー判断待ち。R-39 が Type 1/2 と門番の先行を先に埋めた) **完了(第253)**: [答申](v2-r38-closed-output-judges-research.md)(等級 B・原典 18 群)+親検収 4 件。Jev/Open-Jev は親が別ノート [v2-openjev-note.md](v2-openjev-note.md)(等級 A)。 **第254**: 「RTX 5070 12 GB」の評価軸は撤回(ユーザー指示・資源は順位の理由にしない)。「logprob は受入と衝突」は親が訂正(B14 で logprobs 32/32 一致)。 |
| R-39 | **人の思考のうち判断が占める割合・Type 1/2 の比率と発火条件**(ユーザー問い・第241): 判断の瞬間の割合を測った経験サンプリング・二重過程の定義と介入頻度・速い判断が信頼できる条件・速い層が遅い層を呼ぶ計算モデル/LLM カスケードの先行・社会シミュの例 | [v2-r39-judgment-share-research.md](v2-r39-judgment-share-research.md)(親確認 4) | **完了(第242)**: 直接測った研究は**無い**・最も近い一次値は Hofmann 2012(欲求 49.9%・葛藤 47%・抵抗 42%=派生 約 21%)・引き金は強度でなく葛藤・Type 1/2 は速さでなく作業記憶で定義・Type 2 は既定では発火しない(<20%・CRT 1.24/3)が監視は常時・任せる条件は高妥当性環境+学習機会・系の個数は環境のばらつき×メタ推論コスト(Milli 2017)・FrugalGPT 98%・社会シミュの毎決定ルーティング先行なし・訂正 2(架空語「Hybrid Constitutional Architectures」不在・Klinger & Cox 比率未取得)・空欄 14・lit 18(留置) |
| R-40 | **Jev(TypeSafe System One)の詳細**(ユーザー依頼・第241): API 限界・料金/契約/データ取扱い・アーキテクチャ公開情報・評価再現性・決定論性/日本語/JSON/費用算術・類似 | [v2-r40-jev-details-research.md](v2-r40-jev-details-research.md)(親確認 8) | **完了(第242)**: 文脈長 64k/32k・レート 250,000 tok/s と 1,200 req/min(現行腕 40 分は 104% で超過・120 分なら余裕)・1 リクエスト=1 state・バッチなし・日本語は精度低下明記・jaggedness(context rot・Noul/Choice 非整合 1.19)・決定論の保証なし(温度/seed なし)・CEO「ゼロショット分類器」に同意・非公開アーキテクチャ・**MCA §2.3(f) ベンチマーク公表禁止・Telemetry 無制限・オンプレ不可・SLA なし**・Vercel/Cloudflare 経由の入口・独立検証 2 件・第238 ノート訂正 2・未読 14(FAQ 本文は JS 展開で取得不可)・lit 18(留置) |
| R-41 | **認知アーキテクチャ議事録(個体側)の先行検証**(ユーザー依頼・第255): System 1.5 と再考の引き金・BDI の intention reconsideration・熟慮の System 1 へのコンパイル・LLM エージェントの認知アーキテクチャ・割り込み・注意と知覚 LOD・他者予測と局所回避・Agent LOD | [v2-r41-cognitive-architecture-precedents.md](v2-r41-cognitive-architecture-precedents.md)(親検収・訂正 1) | **完了(第256)**: 等級 B・原典 8・空欄 24。**先行なし 6 件**(6 項の重み和・Policy validity 5 値・valid_until 検査・割り込み 5 状態/3 段・知覚 LOD の意味的 6 段・**Agent 自身の状況で決める LOD**)。**親が逐語 1 件を訂正**(Moussaïd の dominant role) |
| R-42 | **認知・社会アーキテクチャ議事録の先行検証(社会側 §26〜§33・方法論側 §34〜§43)**(ユーザー依頼・第255): 更新順序と同時性・会話の時間差・群の形成・規範の創発・組織・伝播と信頼・創発の測り方・検証 3 層/逆推定・仮説と ablation 行列 | [v2-r42-social-methodology-precedents.md](v2-r42-social-methodology-precedents.md)(親検収 4) | **完了(第256)**: 等級 B・原典 3/抄録 7/二次 29・空欄 18。矛盾 1 件(罰なしの規範創発=部分矛盾へ弱めた)。監査文書 §3 に反映 |
| R-23 第3批 | **D 等級だけを根拠にする決定台帳 18 行**の一次確認(D-86 (a)・第228) | [v2-r23-primary-check-batch3.md](v2-r23-primary-check-batch3.md)(親確認 3: NCP-Bench・0.599^10・cascade 5×) | **完了(第229)**: (a)9/(b)9/撤回 0・訂正 18(3 反映・15 は次)・空欄 21+8・L385 の根拠答申は不在 |
