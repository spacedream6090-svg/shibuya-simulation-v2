...
**Open-Jev は実在する。しかし我々は使えない。** 理由は 3 つで、どれも単独で落ちる。

| # | 理由 | 根拠 |
|---|---|---|
| 1 | **手元のハードに載らない**。vLLM 経路は「24 GB 以上の NVIDIA GPU」が要件で、重みが約 18 GB。**手元は RTX 5070 の 12 GB**(サーバーは 09-22 に喪失) | リポ README |
...
2. **拡散型言語モデルという選択肢の存在は記録しておく**。Apache-2.0 で重みが公開され、ローカルで動き、並列に出力を返す器が現実に存在する。ただし今の v2 には(a) 24 GB 以上の GPU が無い (b) 日本語の裏づけが無い (c) 自己回帰版より精度が低い ので、**今は採らない・将来ハードが戻ったら腕として測る**。
3. **~~logprob を読むのが最も安い~~ → 却下(第253 の R-38 検収で判明)**。書いた時点では「既に持っている Qwen3-8B INT8 の logprob を読めばよい」と考えたが、**vLLM 公式 FAQ が逐語で「vLLM does not guarantee stable log probabilities (logprobs)」と書いている**(親が確認)。v2 は「同 seed で bit 一致」を受入に置いているので**衝突する**。代わりの第一候補は **蒸留 lookup 表**(GenWorld 方式・CPU で 0.54 μs/件・完全に決定論)。詳細は [R-38 答申](v2-r38-closed-output-judges-research.md) と PENDING D-101。
