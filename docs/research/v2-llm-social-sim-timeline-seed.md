...
<!-- hdr:v1 -->
- **分野**: 計算社会科学 #25 | **重要度**: P0(親判断・2026-09-15 第191)
- **一次確認**: **B** = 出典あり・空欄を明示(残務台帳へ写し済みまたは要写し) — 出典痕跡 33 件。空欄節あり
...
- 本文実読: #4・#5・#6・#8・#10(計算費の数値)・#11(PMC 全文)・#12(処方側の記述)・#13(事前登録の推奨の有無)・#15・#17・#18。
  → **第202 完了**(答申 [v2-r2-llm-social-sim-fulltext-check.md](v2-r2-llm-social-sim-fulltext-check.md)・lit 12 本)。#12 は処方側の記述あり(△→✔)。#10 の計算費は Table 3 で 2.6〜5.1 倍(本文に倍率の文は無し)。#13 に事前登録の語は無し。
- 各件の「採用済み / 第2陣で採用 / 該当せず」の確定と、第2陣の決定アジェンダへの写し。
- #14 の実体探索(事前登録を主題にした 2026-06 の論文があるか)。→ 第202: #13(Ye 2605.18890)にも無し。**空欄継続**(事前登録の外部根拠は Larooij & Törnberg 2025 の 1 行のみ)。
