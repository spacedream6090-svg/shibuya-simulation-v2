...
### D-66 / 計画実行層(第1段)(2026-09-11・ユーザー承認 第145・設計書 `docs/design/v2-plan-executor-design.md`)

> **設計書の決定項は 1 つも書き換えていない**。ここに登録するのは、承認済み設計を実装するときに実装側が決めた自前規約(expedient)・切替口・帰無腕・実測値と、**設計書と実装がずれた 6 点**(下の「設計との差」)。新規モジュールは `src/shibuya/engine/presence.py`(層契約: `engine` 内・`world`/`agents` は読むだけ・書き込みは `engine.resolve` の口だけ)。

- **形**: W17 週次表を**在圏ブロック**(居住区分つき定義)に畳み、日次で `(tick, 種, 体, 引数)` のイベント列を組んで、tick ごとに `searchsorted` 1 本で切り出して resolve を叩く。叩く口は `rail_arrive` / `rail_depart` / `begin_planned_sleep` / `wake_from_plan` / `release_indoor` / `balk_queue` / `place_at_external` と、計画欄 2 本の書き込み口 `set_plan_state`(新設・下記「設計との差 5」)。
- **ブロックの定義(derive・§2)**: `place_kind ≠ 域外` ∧ `活動 ≠ 乗車` の**連続行**を 1 本に畳む。**域外居住者(`home_cell < 0`)の「自宅」行は域外**として扱い、域内居住者の「自宅」行は在圏に数える。**親検証値と完全一致**: 全 390,067 体 day0 で 域外居住 346,445 体・在圏ブロック **306,410 本**・体あたり 0 本 **85,766** / 1 本 214,998 / 2 本 45,631 / 3 本 50(アジェンダ §1 の表と 1 体も違わない)。畳みの実測 **0.24 s @390,067 体**(日次構築予算 ≤2 s)。
- **吸収したもの**: rail の乱数 12%(`_build_external_home`)と D-61 帰りの便(`_assign_return`)は `RailProcess(plan_executor=True)` で**止まる**(コードは帰無腕のために残す)。D-62 の `begin_planned_sleep`/`wake_from_plan` の発火元は run.py から層へ移した(**呼ぶ位置は同じ**=「計画境界の起床候補」の所)。hotel の `external_home` は `presence.home_out`(母数 12% → **88.8%**=全 390,067 体で 346,445。mock 5,000 体の標本では 89.8%)、civic の引き込み候補は `presence.candidates_for_pull`(その日在圏予定のある域外の体)。

| # | expedient | 切替口 | 実測・昇格条件 |
|---|---|---|---|
| E1 | 到着便 = ブロック開始に**間に合う最も遅い便**(`enter_tick ≤ start` の右端)。始発前は始発=**遅刻する**。到着がブロック終了を過ぎるならその日そのブロックには**来ない**(到着も退出も出さない) | 現状は定数(`--arrival-train` は未実装=第2陣) | 駅別時間帯乗降の取得(公表なし=PT c-2/b-4) |
| E2 | 方面ノード → 線は W12 `w12_external_nodes.lines[0]`(埼京線/湘南新宿ラインは**埼京線に畳む**)。時刻表に便の無い線はゲート扱い | `ProcessAssets.ext_node_line_idx`(表を差し替え) | 交通センサス駅別流動 |
| E3 | 非鉄道ゲート(方面 8=徒歩 / 9=道路・全体の 2.4%)は所要時間なし=ブロック開始 tick ちょうどに出口セルへ | `--gate-mode`(未実装) | 徒歩・バスの流入時刻分布 |
| E4 | 退出は**即時** `rail_depart`(受容関数を通さない) | `--exit-mode immediate\|board_intent\|walk_to_platform`(**immediate のみ実装**・他は `NotImplementedError` で予約) | D-51 第2陣 |
| E5 | W17 の「乗車」行を在圏に数えない | `PlanBlocks(count_transit=True)` | W17 ブロック形式(§9) |
| E6 | 出勤率。通勤(kind 0)・通学(kind 5)のうち `_mix64(agent_id) % 10000 ≥ rate×10000` の体をその日**終日域外**にする(**乱数を 1 語も引かない**=T4 規模不変・日は混ぜない=毎日同じ体が休む) | `--attendance-rate`(既定 1.0=1 ビットも変わらない) | 国勢調査 昼間就業者概念への較正 |
| E7 | その日の在圏ブロックが 0 本の体は終日域外 | `PlanBlocks` の定義 | 来街頻度の公的分布(現状なし) |
| E8 | 到着分散: 便の受け入れ上限 = `定員 × 混雑率上限`。超過は `agent_row` 昇順で残し、溢れを**より早い便へ** 1 便ずつ送る(上限 8 便・行き場が尽きたら最も早い便が受け切る) | `ARRIVAL_SPREAD_MAX_TRAINS`(=8) | 駅別時間帯乗降 |
| E9 | 計画一致率 = 正時標本の**両側**(在圏側: 計画が在圏と言う体のうち `transit_state==0` / 域外側: 計画が域外と言う体のうち `transit_state≠0`) | 定義を本行へ登録 | — |
| E10 | 降車セル = W11 `w11_station_exits` の出口セル(45 出口 → 25 セル)へ**ブロック索引の round-robin**。ホームセル 8 個への集中を避ける | `ProcessAssets.station_exit_cell` | 構内経路(W11 の駅内グラフ)を通す第2陣 |
| E11 | 到着 tick は**前のブロックの終了より前には戻さない**(戻ると DEPART より先に ARRIVE が来て、その体が域外に出たきりになる) | — | ブロック形式(隙間なし)で不要になる |

- **腕・ablation**: `ablation_id = "AB-PLAN-EXECUTOR"`(層が持つ。`tools/c8/c8lib.scan_engine_ablation_ids` の走査先に `engine/presence.py` を足した=台帳の分母 **21 → 22**)。`run_day(plan_executor=False)` / `--no-plan-executor` = **帰無腕**(現行挙動)。**`--ablate AB-PLAN-EXECUTOR`(または `plan_execution`)でも切れる**——層は `engine/processes/` に居らず `WorldProcessRunner` のトグル表に載らないので、`run_day` が `processes_disabled` を見て `plan_executor=False` に落とす(台帳の「`--ablate <AB-*>` でそのまま切れる」を守るため)。manifest 欄 3 つ: `plan_executor` / `exit_mode` / `attendance_rate`。`--replay` は帰無腕でのみ意味(D-58 と同じ宣言)。
- **バイト(M)**: 個体 SoA **+2 B/体**(`plan_activity` int8 + `plan_flags` int8。**127 → 129 B/体**・実測)。層の私物(SoA 外・日次で作り直す): イベント列 4 配列(mock 5,000 体で 約 24,000 件 × 13 B)+ ブロック索引(390,067 体で 377,402 本 × 約 18 B ≈ 6.8 MB)。**S(恒久記録)は増やさない**(保持窓 1 日)。
- **性能(P)**: 層の phase を `phase_seconds["presence"]` に足した。mock 実資産 5,000 体×1 日の実測 **0.0197 ms/tick**(ラン全体 7.07 ms/tick の 0.28%)。日次構築(畳み+イベント列)は 390,067 体で 0.24 s(畳みだけ)。**逐次ループ宣言(P4)**: ①日次構築の到着分散=**その線の便数**ぶん(4,089 便・起動時 1 回)②tick ごとのイベント種(≤5)③退出繰り延べは**繰り延べ中の体数**ぶんのベクトル演算(Python ループなし)④`notify_pushed_out` は**押し出された体数**ぶん(civic の 200 体級)。**個体数に比例する Python ループは 1 本も無い**。
- **変わった golden**: mock 5,000 体 checkpoint(既定腕 `c96baf82…` → **`a11356ea30dd27a9…`**)/ `tests/engine/test_processes_run.py::test_run_day_on_the_real_world_with_all_first_batch_processes`(「域外からの到着」の担い手が rail → 層へ移ったので、層が立つランでは `presence_counters["arrivals"]` を見る)/ `tests/c8` の台帳分母 21 → 22。**帰無腕の golden は 1 バイトも動かない**(下の退化検査)。
- **退化検査(帰無腕)**: `cli --agents 5000 --seed 1 --world data/world/v2 --no-plan-executor` → checkpoint **`c96baf821c9a046f…`(D-62 の記録と一致)**・呼 37,111・会話 54・保存則 Σ139,575,428 OK・センサス残差 0/PASS・M1 **127 B/体**(計画欄を確保しない)。
- **診断だけを直した後の再実行(2026-09-11)**: 3 腕とも checkpoint が**同一**(`c96baf821c9a046f…` / `a11356ea30dd27a9…` / `5cdcd8cfc45b28c7…`)・呼も同一(37,111 / 38,708 / 38,758)。動いたのは要約の内訳行だけ。

**3 腕の実測**(mock 実資産 5,000 体 × 1 日・seed 1・同一プロセス・同一ツリー。物差しは `tools/c7/presence_yardstick.py`・`--before` は帰無腕の journal):

| 欄 | 帰無腕(`--no-plan-executor`) | 既定 | `--attendance-rate 0.88` |
|---|---:|---:|---:|
| checkpoint 最終 | `c96baf821c9a046f…` | `a11356ea30dd27a9…` | `5cdcd8cfc45b28c7…` |
| M1[B/体] | 127 | 129 | 129 |
| 呼 / 会話セッション | 37,111 / 54 | 38,708 / 32 | 38,758 / 26 |
| 保存則 Σ | 139,575,428 OK | 139,575,428 OK | 139,575,428 OK |
| 日次センサス | 残差 0 / PASS | 残差 0 / PASS | 残差 0 / PASS |
| 壁時計[ms/tick](うち層) | 7.419(—) | 7.073(**0.0197**) | 7.141(0.0199) |
| 移動+密度[ms/tick](P2 ≤5) | 0.098 | 0.096 | 0.097 |
| 域外居住 / 終日域外 | 600(乱数 12%) | **4,491 / 1,107** | 4,491 / 1,431(うち出勤率で 334) |
| 到着 / 退出 | 2,091 / 1,876(rail) | **4,491 / 3,460**(層) | 4,130 / 3,143(層) |
| 計画一致率 在圏 / 域外 | — | **0.870 / 0.990** | 0.863 / 0.991 |
| 在圏(bbox)03 / 09 / 12 時 | 4,427 / 4,828 / 4,423 | **670 / 2,858 / 2,588** | 651 / 2,564 / 2,308 |
| 昼夜比 09/03(5 エリア・物差し) | 1.194 | **7.268** | 6.912 |
| 昼夜比 12/03(5 エリア) | 1.083 | 6.588 | 6.219 |
| 09 時 5 エリア(換算・現実 130,167 / PT 145,000) | 264,075 | **164,998** | 147,757 |
| 03 時 5 エリア(換算・推定帯 1.5〜3 万) | 221,090 | **22,702**(帯の中) | 21,376(帯の中) |
| 03 時 D-66 種別の全体比 | 0.8853 | **0.426** | 0.394 |
| 計画就寝 その場 / 着いて | 1,193 / 602 | 33 / 39 | 31 / 42 |
| **域外居住なのに舞台内で就寝**(直接計測) | —(計測なし) | **1 体** | **0 体** |
| 起床率 03 時(D-62 定義=域外も「起きている」) | 0.250 | 0.931 | 0.931 |
| **起床率(在圏)03 時**(新設・a(3)=0.035) | —(層なし) | **0.485** | 0.473 |
| 起床率 12 時(D-62 定義) | 0.743 | 0.994 | 0.994 |
| **起床率(在圏)12 時**(a(12)=0.978) | —(層なし) | **0.989** | 0.988 |
| 到着分散で動かした便 | — | 0(5,000 体では 1 便も上限に達しない) | 0 |
| civic 引き込み / 押し出し(張り直し) | 200 / 200 | 200 / 200(175) | 200 / 200(176) |

- **合格線(親案)への到達**: 昼夜比 ≥5 → **到達**(7.27・5 エリア。bbox 全体では 4.27=**到達せず**)。03 時の域外常住種別 ≤5% → **到達せず**(0.426。絶対値は 195,736 → 9,674 換算で −95% だが、分母=深夜在圏が 1/9.7 に縮んだのでシェアは下がりきらない。設計 §1 の D-66 想定でも 6,007/49,017 = 0.123 なので、≤0.05 は W16 の夜間在圏(域内居住 43,622 体)側を直さないと届かない)。職場就寝 0 → **ほぼ到達**(直接計測 1 体・出勤率腕では 0 体。残る 1 体は W17 がその体の就寝行に**舞台内のセルを解決している**=W16 の `home_cell < 0` と矛盾する行)。起床率 03/12 時 ±0.05 → **12 時は到達(+0.011)・03 時は到達せず(+0.450)**(下の在圏版の物差しで測った値)。
- **起床率の物差しを 2 本にした(2026-09-11・親指示)**: `wake_rate_by_hour`(`起床率/時`)は D-62 の定義で「**域外滞在・乗車中を『起きている』に数える**」(§8 D-62 の観測欄)。層が入ると深夜は 89% の体が域外に居るので、この欄は自動的に 0.93 へ上がる=a(h) と**分母が揃わない**。そこで **要約に 1 行 `起床率(在圏)/時 00:… 23:…` を足した**(`RunResult.wake_rate_in_area_by_hour`・定義=`transit_state == 0` の体のうち `activity != SLEEPING`・層が休んだランは出ない)。`tools/c7/presence_yardstick.py` は**この行があれば a(h) との比較にそちらを優先**し、md/json に定義(`basis` / `basis_note`)と「before は定義が違う」を明記する。**既存の `起床率/時` 行は 1 文字も変えていない**(before のような古い入力は従来どおり読める)。実測: 03 時 0.931 →(在圏版)**0.485**・最大絶対差 0.896 → **0.458(23 時)**・12 時差 +0.016 → **+0.011**。それでも a(3)=0.035 には遠く、残差は W17 の深夜配置(D-63 (iii))と「隙間で起きたまま」の扱い=**層の外**。
- **`begin_planned_sleep` の計数を直した(2026-09-11・親指示・挙動は不変)**: 旧実装は内訳の全てに `ok_cell`(就寝地セルが有効か)を掛けていたので、**就寝地セルの無い域外の体**が `outside` ではなく `unreachable` に落ちていた(既定腕で「行けない 5,976」)。内訳は `transit_state` を先に見る形にし、`unreachable` は「**在圏**なのにセルが無い/経路が無い」体だけにした。実測(既定腕): `行けない 5,976 → 0` / `域外 42 → 6,017` / `就寝済 494 → 495`(在圏で `cell<0` かつ就寝済だった 1 体)。**帰無腕の内訳は 1 件も動かない**(`home_cell < 0` の体が居ないため)。**3 腕の checkpoint は 1 ビットも変わらない**(下の再実行)。
- **事前登録との差**: 09 時の絶対水準は「ほぼ不変」と事前登録したが、実際は 264,075 → 164,998(−37.5%)と**改善した**。機構は明らか(帰無腕では域外居住 88.8% が 24 時間ずっと在圏に数えられており、09 時の分子にも入っていた)。偶然ではないが、**D-67(昼の 1.6〜1.9 倍)が消えたわけではない**(まだ現実 130,167 の 1.27 倍)。
- **テスト**: 新規 `tests/engine/test_presence_executor.py` **21 本**(乱数不使用の AST 検査+イベント列のバイト一致 / 帰無腕で `c96baf82…` 一致 / T4 規模不変 / 保存則を全 tick(960 tick × 30 体)/ 同 seed 2 ラン一致 / 到着が間に合う・始発前は始発・終了を過ぎる到着は捨てる / 日 2〜3 回の出入り / 非来街日は終日域外 / 域外居住者は舞台内で寝ない / 退出の優先規則(会話=繰り延べ・買物=中断・行列=離脱)と 30 tick の強制退出 / 二重到着・二重退出なし / civic 引き込み後に古い到着が発火しない・押し出しは張り直す / 到着分散が容量を超えない / **derive の畳みが親検証値 306,410・85,766 と一致** / `native` と他の `--exit-mode` は予約 / 出勤率の決定論 / `_mix64` が W17 側と同式 / `plan_flags` のビット / 合成世界では層が休む / 要約 1 行が `c7lib.parse_run_summary` に当たらない)。加えて `tests/c7/test_presence_yardstick.py` に 2 本(`起床率(在圏)/時` の優先とmd/json の定義明記)・`tests/c8/test_ablations_table.py` に 1 本(manifest 白リストが腕 3 欄を通す)。
- **設計との差(設計書は書き換えていない・親判断待ち)**:
  1. **SoA の 2 欄は腕つき**。`AgentState(plan_columns=True)` のランだけ確保する。設計書 §2 は常設(127 → 129 B/体)を想定しているが、`Registry.state_hash` は**宣言順の全配列**を混ぜるので欄を足すと**帰無腕の checkpoint も動く**。親指示「帰無腕は 1 バイトも変えない」を優先した。
  2. **イベント種 4 WORK_START / 5 AT_HOME は未実装**。どちらも `target_node` を書く必要があるが、`resolve` にその口が無く、設計書 §11-8「層が世界の口を増やさない」と両立しない。**第2陣**(勤務先・自宅への移動は現状 LLM の「移動」が担う)。
  3. **`_schedule_with_population` の `home_cell<0` 保持と `np.clip` 廃止は層が立つランだけ**(`keep_outside_home=plan_exec_on`)。同じ理由(帰無腕のバイト不変)。`work_cell` の clip は両腕で残した(母集団の `work_cell` に −1 は出ないので挙動差はない)。
  4. **`tests/engine/processes/test_rail_return.py`(15 本)は無改造で PASS**。設計書 §7 は「fixture に `plan_executor=False` を足す」としていたが、あれらは `RailProcess` を直に組むテストで既定が `plan_executor=False` なので手を入れる必要がなかった。`tests/engine/test_plan_sleep_d62.py`(14 本)も無改造(合成世界=層が休む)。
  5. **計画欄の書き込み口 `resolve.set_plan_state` を 1 本新設**した。`plan_activity`/`plan_flags` は SoA なので `resolve` 以外から書けない(運用設計書 §2.2)。設計書 §11-8 が数えた「既存 6 口」には無い口だが、**世界状態ではなく計画の写し**であり、`writable()` を呼ぶのは従来どおり `resolve.py` だけ(AST 検査は不変)。
  6. **`plan_activity` は書くが、まだ誰も読んでいない**(計画一致率はブロック索引から直接計算している)。知覚・civic が読むのは第2陣。

#### 第2段(2026-09-11・層2 独立検収=条件付き合格 → 親指示の 8 点)

> 帰無腕 `--no-plan-executor` の checkpoint **`c96baf821c9a046f…` は不変**(再実行で確認)。既定腕と 0.88 腕は挙動が変わるので checkpoint も変わる(新値は下表)。すべて決定論・乱数を 1 語も引かない。

| # | 直したこと | 実測(mock 実資産 5,000 体×1 日) |
|---|---|---|
| 1 | **域外の体に LLM 呼を出さない**(`outside_suppression`・既定 True)。アービタに D-56 と同型の抑止列を足した(`ArbiterDecision.outside_suppressed`・診断列 `outside_suppressed`・帰無腕 `--no-outside-suppression`)。**例外なし**(計画境界・顕著行為・会話ターンも落とす)——舞台に居ない体は B0-B6 の描画欄が全部空で、プロンプトが「どこにも居ない」になるため。**層が立つランでだけ効かせる**(`--no-plan-executor` は現行挙動=checkpoint 不変の約束) | 既定腕の呼 **38,708 → 25,144**(−35%)・**域外宛ての呼 0**(抑止 28,392)。帰無腕は 37,111 のまま(域外宛て 3,956 は現行挙動として残る) |
| 2 | **在圏なのに `cell = -1`** を潰した。`home_cell < 0` かつ 0 時から在圏ブロックを持つ体を `PlanExecutor.initialize` が出口セルへ `rail_arrive` する | 390,067 体で **559 体**(層2 報告と一致)・mock 5,000 体で 8 体。**全 tick で「在圏 ⇒ `cell ≥ 0` ∧ `node ≥ 0`」** を検査するテストを新設(保存則テストの隣) |
| 3 | **域内居住者(住民)の隙間は退出ではない**(設計書 §10-7 (a))。`home_in` は**次の行(乗車行は飛ばす)が「域外」のときだけ** DEPART。`home_out` はブロック終了=退出のまま | 21 時の住民 在圏 **171 → 184**(帰無腕 297)。乗車行を飛ばすのは E5 との整合(自宅 → 乗車(駅) → 域外 で「次=乗車」と読むと住民が一生退出しない) |
| 4 | **LLM の乗車/退去で域外へ出た体の張り直し**(§4)。`rail.step` の発車ブロックが `presence.notify_departed_by_llm` を叩き、進行中ブロックの DEPART を落として**次のブロックの開始**に ARRIVE を張り直す | `departed_by_llm` 1,196・`depart_dropped` 897 → **`depart_skipped` 758 → 5**(取り残しがほぼ消えた) |
| 5 | **civic の押し出しは進行中ブロックに張り直さない**(翌 tick に戻る事故)。`notify_pushed_out` も #4 と同じ `_rearm_after_leaving` を通す | 退場 200 体の張り直し **175 → 0**(次のブロックが無い体が大半)。civic の実経路(`LargeEventProcess._arrive/_leave` → 層)を通すテストを 1 本追加 |
| 6 | **予算**: (a) 未宣言のイベント種 `PLAN_ACTIVITY` を**削除**(390k で 1.8M 件・誰も読まない)。`plan_activity` は ARRIVE / SLEEP / WAKE の時点で更新する(行の被覆は同じ)。(b) `_build_events` の件数比例ループ(`rank` の内包表記)を**種ごとの定数ブロードキャスト**に置換 | 下の「P/M の再宣言」 |
| 7 | **到着分散 E8 を両側に**した。① 早い便へ(間に合う側・第一)→ ② 置き切れない分を遅い便へ(遅刻・第二)→ ③ それでも置けない分はその便が受け切る | 390,067 体で **上限超えの便 0 本**・最悪便 **埼京線 08:58 便 2,405 体 = 混雑率 162.9%**(前 401%・埼京 08:41 便 6,641 体)。**目標 200% 未満を達成**。散らした体 178,954・受け切り 0 |
| 8 | **診断の定義**: `home_out_slept_inside` から場所種別「宿泊施設」を除く(ホテル泊は正当) | 既定腕 **1 → 0 体**・390k でも 0 体 |

**P/M の再宣言(390,067 体・層単独 1 日の実測・LLM なし)**

| 欄 | 宣言(第1段) | **実測(第2段)** | 判定 |
|---|---|---|---|
| 層の壁時計 平均[ms/tick] | ≤0.3 | **0.4126** | **届かない**(目標比 1.38 倍)。内訳は 18:00 の退出 tick |
| 同 p99 / 最大[ms/tick] | (宣言なし) | **15.88 / 28.19** | 最大は tick 1,080(18:00)の DEPART **61,364 件**=1 tick に集中する W17 の丸め |
| 層の 1 日合計[s] | — | **0.59**(ラン全体 1,440 tick の 0.04%) | 予算内 |
| 日次構築[s] | ≤2 | **0.87**(+ 起動配置 0.08) | OK |
| イベント列 | ≈21 MB | **36.7 MB**(2,448,789 件 × 15 B) | 超過(`PLAN_ACTIVITY` を削って 1.8M 件・29 MB を落とした後の値) |
| ブロック索引 + 到着表 | ≈3.7 MB | **8.8 + 9.1 = 17.9 MB** | 超過 |
| 層の私物 合計 | ≈25 MB | **54.6 MB**(M8 の 0.23%) | **超過・実測で再宣言** |
| 個体 SoA | 129 B/体 | **129 B/体** | OK |

**3 腕の実測(第2段・mock 実資産 5,000 体×1 日・seed 1)**

| 欄 | 帰無腕 `--no-plan-executor` | 既定 | `--attendance-rate 0.88` |
|---|---:|---:|---:|
| checkpoint 最終 | **`c96baf821c9a046f…`(不変)** | **`34cd8c019f7eb52d…`** | **`d600ec47d4eaa81c…`** |
| 呼(第1段 → 第2段) | 37,111 → **37,111** | 38,708 → **25,144** | 38,758 → **23,959** |
| 域外宛ての呼 / 域外抑止 | 3,956 / 0(抑止 off) | **0 / 28,392** | **0 / 29,569** |
| 保存則 / 日次センサス | OK / 残差 0 PASS | OK / 残差 0 PASS | OK / 残差 0 PASS |
| 壁時計[ms/tick](うち層) | 7.265(—) | **5.296**(0.0217) | 5.167(0.0219) |
| 到着 / 退出 / 取り残し退出 | 2,091 / 1,876 / — | 4,313 / 3,096 / **5** | 3,946 / 2,806 / **1** |
| 計画一致率 在圏 / 域外 | — | 0.834 / 0.984 | 0.825 / 0.985 |
| 昼夜比 09/03(5 エリア) | 1.194 | **6.570** | 6.055 |
| 09 時 5 エリア(換算・現実 130,167 / PT 145,000) | 264,075 | 160,942 | **145,027** |
| 03 時 5 エリア(換算・帯 1.5〜3 万) | 221,090 | 24,496(帯内) | 23,950(帯内) |
| 03 時 D-66 種別 全体比 | 0.8853 | 0.452 | 0.417 |
| 起床率(在圏)03 / 12 時 | —(層なし) | **0.485 / 0.983** | 0.464 / 0.979 |
| 域外居住なのに舞台内で就寝 | — | **0 体** | **0 体** |
| 21 時の住民 在圏 | 297 | **184** | 187 |
| 近づいた指標(yardstick) | 0/20 | **18/20** | **18/20** |

- **合格線の到達(第2段)**: 昼夜比 ≥5 → **到達**(6.57)。03 時の域外常住種別 ≤5% → **到達せず**(0.452)。職場就寝 0 → **到達**(0 体)。起床率(在圏)±0.05 → **12 時 到達(+0.005)・03 時 到達せず(+0.450)**。
- **未対処(宣言)**: ① **18:00 の DEPART が 1 tick に 61,364 件**(390k・W17 の開始/終了が :00/:30 に丸まるため)。層の p99/最大 ms/tick の主因。**今回は触らない**——W17 をブロック形式(分単位)で再生成すれば解消する見込み(設計書 §9)。② **訪日来街者に就寝行が無く毎晩域外へ出るのでホテル過程が死ぬ**(泊 1 → 0)。W17 再生成の制約「宿泊客の就寝地=宿泊施設」を入れるかは**ユーザー判断待ち**=**既知・未対処**。
- **変わった golden(第2段)**: `tests/c6/test_t5_order_bias.py` の `_run` に **`plan_executor=False`** を足した(`sleep_suppression=False` / `plan_sleep=False` と**同じ理由**)。既定だと tick 0 に域外常住 88.8% が舞台の外へ置かれ、域外抑止でその候補が全部落ちるので、24 tick の窓で呼ばれるのは在圏の住民・従業者にほぼ限られる=**構成のバイアス**が層内 |ρ| に入る(実測 `mean_wake_tick` 0.0960・`selected_count` 0.0626 > 0.05・呼 826 のうち 648 が RESIDENT)。T5 が測るのは `arbitrate` の**並び順**のバイアスなので帰無腕で回す。**T5 の判定を W17 + 計画実行層込みにするかは親判断=PENDING D-64 と同じ論点**。
- **設計との差(追加)**: **域外抑止は層が立つランでだけ効く**。`--no-plan-executor` の帰無腕では rail の乱数 12% で外に居る 600 体にも従来どおり呼が出る(親指示「帰無腕の checkpoint c96baf82 は不変」を優先した)。

#### 第3段(2026-09-11・層2 再確認=条件付き合格 → 親指示の 3 点)

> 帰無腕 `--no-plan-executor` の checkpoint **`c96baf821c9a046f…` は不変**。既定腕・0.88 腕は挙動が変わるので新値(下表)。決定論・乱数なし。

1. **E8 の窓を厳密化(重)**。第2段の実装は①(早い便へ)で hop 上限に達した体を**置かずに運び続け**、j=0 で前向きパスへ落として**始発から前詰め**していた(390,067 体で |便差| 最大 191 便・09:00 開始ブロックの 21,418 体が 06 時前に到着・01〜05 時の在圏 +9k=層2 実測)。直した形: ① は **E1 便から 8 便**離れた時点で①を降り、② は**自分の E1 便の 1 つ後ろ**から遅刻側へ 8 便まで、③ それでも置けない体は**自分の E1 便が受け切る**。⇒ **|到着便 − E1 便| ≤ 8 が不変条件**(テストが全数検査)。
2. **E12 進行中ブロックへの復帰(中)**。`_rearm_after_leaving` は「次のブロック(`start > tick`)」にしか張らないので、**その日 最後のブロックの途中で LLM が乗車を選んだ体は当日帰れなかった**(層2 実測: 日末に「計画では在圏なのに域外」141/509・`rearmed` 1/1,194)。直した形: **次のブロックが無く**進行中ブロックの残りが `RETURN_MIN_AWAY_MIN`(=60 分)以上あるなら、`tick + 60` 以降に**最も早くホームへ入る便**で戻す(線は選ばない=渋谷は 1 駅・D-61 と同じ宣言)。**戻る体の DEPART は落とさない**(ブロック終了で正しく退出する)。**civic の押し出しはこの復帰を使わない**(翌 tick 復帰の再発防止)。診断 `plan_stranded_eod` を要約に出す。
3. **`arrive_skipped` の内訳(軽)**: 「元から在圏(I2)」と「civic に引き込まれた後」に分けた(`arrive_skipped_in_area` / `arrive_skipped_pulled_in`)。

| # | expedient | 切替口 | 昇格条件 |
|---|---|---|---|
| E12 | LLM の乗車/退去で出た体の**進行中ブロックへの復帰**(次のブロックが無いときだけ・最短外出 60 分・`t+60` 以降の最も早い便) | `RETURN_MIN_AWAY_MIN`(定数) | PT の私事トリップの**滞在時間分布** |

**390,067 体・層単独 1 日の実測(第2段 → 第3段)**

| 欄 | 第2段 | **第3段** | 判定 |
|---|---:|---:|---|
| \|到着便 − E1 便\| 最大 / ≤8 の割合 | 191 / —(層2 実測) | **8 / 1.000**(n=324,912) | **窓を守る** |
| 同 p50 / p90 | — | **1 / 5** | |
| ブロック開始までのリード p50 / p90 / 最大[分] | — | **3 / 11 / 298** | 到着がブロック開始より後(遅刻=②の前向き溢れ)は 28,779 件 |
| 在圏 01 / 02 / 03 / 04 / 05 時 | 03 時 66,169 | **55,064 / 55,405 / 55,348 / 55,144 / 55,501** | 03 時 **−10,821**(層2 の「+9k」が解消) |
| 在圏 06 / 07 / 08 時 | — | **62,592 / 89,877 / 110,087** | 層2 の「08 時 +32k」は早着が消えたぶん下がる |
| 在圏 09 / 12 時 | 255,747 / 257,584 | **240,719 / 257,584** | |
| 最悪便の混雑率 | 162.9%(埼京 08:58 便 2,405 体) | **162.9%(同)** | 200% 未満を維持 |
| 上限超えの便 / 受け切り | 0 本 / 0 体 | **0 本 / 0 体** | |
| 層 平均 / p99 / 最大[ms/tick] | 0.4126 / 15.88 / 28.19 | **0.4163 / 15.73 / 27.16** | 目標 ≤0.3 に**届かない**(据え置き) |
| 日次構築[s] / 層の私物[MB] | 0.872 / 54.6 | **0.866 / 54.6** | |
| 在圏なのに cell 欠損 / 日末の計画在圏なのに域外 | 0 / — | **0 / 0** | |

**3 腕の実測(第3段・mock 実資産 5,000 体×1 日・seed 1)**

| 欄 | 帰無腕 `--no-plan-executor` | 既定 | `--attendance-rate 0.88` |
|---|---:|---:|---:|
| checkpoint 最終 | **`c96baf821c9a046f…`(不変)** | **`b66245d9c9b262ef…`** | **`d4a067fb1b2d55ce…`** |
| 呼(第2段 → 第3段) | 37,111 → **37,111** | 25,144 → **26,496** | 23,959 → **25,577** |
| 域外宛ての呼 / 域外抑止 | 3,956 / 0(抑止 off) | **0 / 27,331** | **0 / 28,494** |
| 保存則 / 日次センサス | OK / 残差 0 PASS | OK / 残差 0 PASS | OK / 残差 0 PASS |
| 壁時計[ms/tick](うち層) | 7.277(—) | 5.581(0.0362) | 5.492(0.0346) |
| 到着 / 退出 / 取り残し退出 | 2,091 / 1,876 / — | 5,304 / 3,817 / **5** | 4,841 / 3,492 / **2** |
| **進行中ブロックへの復帰(E12)** | — | **1,003** | 904 |
| **日末の計画在圏なのに域外**(`plan_stranded_eod`) | — | **62**(層2 before 141) | 71 |
| 計画一致率 在圏 / 域外 | — | **0.970**(第2段 0.834)/ 0.982 | 0.971 / 0.983 |
| `arrive_skipped`(元から在圏 / 引き込み後) | — | 130(101 / 29) | 128(99 / 29) |
| 昼夜比 09/03(5 エリア) | 1.194 | **6.799** | 6.345 |
| 09 時 5 エリア(換算・現実 130,167 / PT 145,000) | 264,075 | 166,559 | **151,970** |
| 03 時 5 エリア(換算・帯 1.5〜3 万) | 221,090 | 24,496(帯内) | 23,950(帯内) |
| 03 時 D-66 種別 全体比 | 0.8853 | 0.452 | 0.417 |
| 起床率(在圏)03 / 12 時 | —(層なし) | 0.485 / **0.986** | 0.464 / 0.984 |
| 21 時の住民 在圏 | 297 | **252**(第2段 184) | 253 |
| 近づいた指標(yardstick) | 0/20 | **18/20** | **18/20** |

- **合格線(第3段)**: 昼夜比 ≥5 → **到達**(6.80)。職場就寝 0 → **到達**(0 体)。起床率(在圏)12 時 ±0.05 → **到達**(+0.008)。03 時 → **到達せず**(+0.450)。03 時の域外常住種別 ≤5% → **到達せず**(0.452)。
- **未対処(据え置き)**: 18:00 の DEPART 61,364 件/1 tick(層の p99/最大の主因・W17 をブロック形式へ再生成すれば解消見込み)/ 訪日来街者に就寝行が無くホテル過程が死ぬ(泊 1 → 0・**ユーザー判断待ち**)/ 層の平均 ms/tick が目標 0.3 に届かない(実測 0.4163)。
- **新規テスト(第3段・2 本)**: E8 の「\|到着便 − E1 便\| ≤ 8」を 400 体 30 便で全数検査 / E12 の「次のブロックが無い体は `t+60` 以降の便で戻る・civic の押し出しは戻らない」。
