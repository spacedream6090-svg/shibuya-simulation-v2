...
| 広告ゼロ | AD1 封印行の裏=看板行を全セル空にして行動分布の JSD | ⑥ 完走(seed 1・2): 購入 −1.34/−1.02 pp・休憩 +1.22/+1.06(帰無 ≤0.22 pp)=再現する実効果・AD1 の 1pp 線を超える(D-59) |
...
**⑥ の読み方(親)**: AD1 封印行 S3 は「広告全消去で購入は絶対 1pp 未満しか動かない」を現実側の期待として置いている。sim 側は **−1.34 pp(seed 1)/−1.02 pp(seed 2)** で線を超える。同一構成の seed 差(n=50,000)は購入で 0.16 pp なので、超過は seed 揺らぎでは説明できない(上の表)。メモリ「LLM の広告過剰反応が最大リスク」に対応する最初の実測=**D-59**。評価は購入だけでなく休憩(+1.22 pp)も動いた点を含める(看板行が消えると休む=注意配分の代替先)。
