...
2. **pytest 再実行**: `.venv/Scripts/python.exe -m pytest -q -p no:cacheprovider` を自分で実行し、passed/failed 件数を写す。作業役の貼った出力と件数が一致するか。
...
9. **ブランチ**: 作業が `build/c0-c4` にあり main に触れていない(`git branch --show-current`)。
