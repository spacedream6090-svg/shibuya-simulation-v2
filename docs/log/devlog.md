...
## 第246 AB6c seed 1(無制限×看板)完走・親検収(2026-09-20 朝)

- **ラン**: 00:19〜02:46(4 腕・37 分/腕=上限ありと同じ壁時計)。回収→ `docs/bench/c8/ablation/ablation_AB6c-AD-NOTICE-UNCAPPED_s1.{json,md}`・runner log・テープ 4 本(gitignore)。
- **親検収**: 保存則/センサス OK・繰り延べ 0・テープ再集計(段0 辞書込み)全語一致・注視率 0.3007/0.1417。[親報告](../bench/c8/ablation/ab6c_s1_parent_report.md)。
- **結果**: 呼数 36.9k→77.3k(2.1 倍・15.5/体/日)で止まる(抑止は就寝/域外/不応期で予算ではない)。増分はセル/個体起床。購入 % 43.9→36.4 は分母(起床構成)の変化=D-98 の実例。**看板の購入効果はゼロ**(|Δ| ≤ 0.11 pp)=上限ありの +0.3〜0.5 pp は予算の影。**休憩 +2 pp・会話 ×2.6 は無制限でも残る**=文面の注意配分の効果。実現量: 看板なしで乗車 +23%・購入 +0.9%・売上 +1.4%。
- **次**: AB8(0.5×/1×/2×/無制限)実行中→回収・親検収。D-59 は閉じる推奨(据え置き)。

## 第247 AB8 seed 1(L4 感度腕)完走・親検収(2026-09-20 朝)

- **ラン**: 02:47〜04:54(4 腕・2 h 07 m)。回収→ `docs/bench/c8/ablation/ablation_AB8-L4-SCALE_s1.{json,md}`・テープ 4 本(gitignore)。連鎖 CHAIN_245_DONE・サーバー空き。
- **親検収**: 保存則/センサス OK・再集計 全語一致・**×1 の final_hash と呼数・購入が AB6b s1 の baseline(別コード版)と bit 一致**・無制限は AB6c s1 と一致。[親報告](../bench/c8/ablation/ab8_s1_parent_report.md)。
- **結果**: 実現購入/体/日 0.78→1.32→1.84→1.92(×2 で飽和・現行 69%)・売上 630→1,632 円/体・乗車は 648→407 と減る・購入 % 50→36 は起床構成の差・会話は無制限でも 31 セッション。**L4 は結果を駆動している**。
- **判断待ち(新)**: D-99 上限の置き方(親推奨 (a) 5,000 体の腕は無制限・40 万体は ×2)。D-59 閉じる推奨は据え置き。
- **次**: ユーザー決定。図 8(予算 vs 実現量)は要望があれば tools/fig に追加。

## 第248 検証ラン(AB6c/AB8)の図(2026-09-20 朝)
