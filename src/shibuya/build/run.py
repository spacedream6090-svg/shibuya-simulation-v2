...
ALL_STAGES: tuple[str, ...] = tuple(
    stage_order([*GEO_STAGES, *FIELD_STAGES, *VIS_STAGES, *AUDIT_STAGES])
)
...
#: (W18 は全段階のヘッダを、W19/W20 は W18 の出力を読む)。
RUN_ORDER: tuple[str, ...] = (
    *[s for s in ALL_STAGES if s not in VIS_STAGES and s not in AUDIT_STAGES],
    *VIS_STAGES,
    *AUDIT_STAGES,
)
