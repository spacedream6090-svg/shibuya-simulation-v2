"""build.vis.run — 可視性・影レーンの段階を W 番号順に実行する CLI。

    python -m shibuya.build.vis.run --out data/world/v2 [--stage W8] [--data data]

- 各段階は純関数(§0-1)。出力とヘッダ ``<stage>.header.json`` を ``--out`` に書く。
- 本 CLI は **manifest を書かない**(build_hash は全段階分をまとめて書く
  ``python -m shibuya.build.run`` の仕事)。
- ゲートが1つでも落ちたら終了コード 1。
"""
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="python -m shibuya.build.vis.run")
