...
C = {
 ...
 'v2-r38-closed-output-judges-research.md':    ([27,29,31], 'P0', 'R-38 閉じた出力の判断層(第253): NLI ゼロショットの絶対精度は低い(Yin 2019 topic 45.7)が日本語モデルは実在(Formzu JSNLI 0.9288・親確認)。vLLM 公式は logprob の安定性を明示的に非保証=bit 一致の受入と衝突。自己申告 confidence での振り分けは ECE で批判あり(Xiong ICLR2024)。効くのは別学習のスコアラ(FrugalGPT DistilBERT)。社会シミュで毎決定を蒸留代理に置換した先行は在る(Light Society 10 億体・GenWorld 日本東広島 196,608 体を CPU 0.54us/件の lookup 表へ=親が本文逐語確認)が、実行時に学習ルータが振り分ける先行は無い=R-39 は狭い意味で維持'),
